...
public class LocalPropertyEnvironmentPostProcessor implements EnvironmentPostProcessor, ApplicationListener<ApplicationEnvironmentPreparedEvent> {
    ...
    public void postProcessEnvironment(ConfigurableEnvironment environment, SpringApplication application) {
        final FileSystemResource resource = new FileSystemResource("deploymanager-local.properties");

        if (resource.exists()) {
            Properties loadAllProperties = PropertiesLoaderUtils.loadProperties(resource);
            environment.getPropertySources().addFirst(new PropertiesPropertySource("file:deploymanager-local.properties", loadAllProperties));
