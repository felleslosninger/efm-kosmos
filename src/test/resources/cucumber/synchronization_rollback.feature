Feature: Synchronization - Rollback
  ...
  Background:
    ...
    And a "GET" request to "/maven2/no/difi/meldingsutveksling/integrasjonspunkt/2.2.1" will respond with status "200" and the following "application/java-archive" in "/cucumber/failure.jar"
    ...
    """
    And a "GET" request to "/maven2/no/difi/meldingsutveksling/integrasjonspunkt/2.2.1.jar.asc" will respond with status "200" and the following "text/plain" in "/cucumber/failure.jar.asc"
